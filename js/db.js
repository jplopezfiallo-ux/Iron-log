...
const DB_VERSION = 1;
const SCHEMA_VERSION = 2; // app-level record shape version, independent of IDB version
...
const DEFAULT_EXERCISES = [
  ...
  { name: 'Romanian Deadlift', icon: 'romanian-deadlift' },
  { name: 'Squat', icon: 'squat' },
  { name: 'Leg Press', icon: 'leg-press' },
  { name: 'Hip Abductor Machine', icon: 'hip-abductor' },
  { name: 'Leg Extension Machine', icon: 'leg-extension' },
  { name: 'Hamstring Curl Machine', icon: 'hamstring-curl' },
  { name: 'Crunches', icon: 'crunches' },
];
...
async function seedIfEmpty() {
  ...
  // Existing install — add any default exercises introduced in a later
  // version (matched by name) without touching what's already there.
...
window.IronLogDB = {
  DB, uid, seedIfEmpty, exportAllData, importAllData, SCHEMA_VERSION,

...
const { DB, uid, seedIfEmpty, exportAllData, importAllData } = window.IronLogDB;
...
function profileOnboardingScreen() {
  ...
  wrap.appendChild(profileForm(async (name, color) => {
    await DB.put('profiles', { id: uid(), name, color, createdAt: Date.now() });
    await loadAll();
    render();
  }));
  return wrap;
  ...
  state.profiles.forEach((p) => {
    ...
    editBtn.onclick = (e) => {
      ...
      content.appendChild(profileForm(async (name, color) => {
        await DB.put('profiles', { ...p, name, color });
        await loadAll();
        overlay.remove();
        render();
      }, p));
      ...
      delBtn.onclick = async () => {
        ...
        const sets = await DB.getAllByIndex('sets', 'profileId', p.id);
        for (const s of sets) await DB.delete('sets', s.id);
        const sessions = await DB.getAllByIndex('sessions', 'profileId', p.id);
...
function openRoutineModal() {
  ...
  saveBtn.onclick = async () => {
    ...
    await DB.put('routines', { id: uid(), name, exerciseIds: [...selected], createdAt: Date.now() });
    await loadAll();
    overlay.remove();
    render();
  };
...
function historyScreen() {
  ...
  mySessions.forEach((session) => {
    ...
    delBtn.onclick = async () => {
      ...
      for (const s of sessionSets) await DB.delete('sets', s.id);
